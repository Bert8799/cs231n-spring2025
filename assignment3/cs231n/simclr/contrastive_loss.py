...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        def compute_loss(z_a, z_b):
            # Compute the numerator value for the positive pair (z_a, z_b).
            numerator = torch.exp(sim(z_a, z_b) / tau)

            # Compute the denominator value.
            denom = 0
            for i in range(2*N):
                if not torch.equal(z_a, out[i]):  # skip the case when i == a
                    denom += torch.exp(sim(z_a, out[i]) / tau)

            loss = -torch.log(numerator / denom)
            return loss
        loss_1 = compute_loss(z_k, z_k_N)
        loss_2 = compute_loss(z_k_N, z_k)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    dot_products = torch.sum(out_left * out_right, dim=1)  # [N]
    norms_left = torch.linalg.norm(out_left, dim=1)  # [N]
    norms_right = torch.linalg.norm(out_right, dim=1)  # [N]
    pos_pairs = dot_products / (norms_left * norms_right)  # [N]
    pos_pairs = pos_pairs.view(-1, 1)  # [N, 1]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # sim(i,j) = sim(j,i), so we need to repeat the entries.
    numerator = torch.exp(pos_pairs / tau).repeat(2, 1)  # [2*N, 1]
