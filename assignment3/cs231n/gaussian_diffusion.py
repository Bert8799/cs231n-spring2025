...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        #############################################################################
        # alpha_bar_{t-1}
        alphas_cumprod_prev = nn.functional.pad(
        ...
        loss_weight = torch.ones_like(
            snr) if objective == "pred_noise" else snr
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        x_start = (x_t - sqrt_one_minus_alphas_cumprod_t *
                   noise) / sqrt_alphas_cumprod_t
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        sqrt_one_minus_alphas_cumprod_t = extract(
            self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        pred_noise = (x_t - sqrt_alphas_cumprod_t * x_start) / \
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #      and sample x_{t-1}.
        ##################################################################
        model_prediction = self.model(x_t, t, model_kwargs)
        if self.objective == "pred_noise":
            pred_x_start = self.predict_start_from_noise(
                x_t, t, model_prediction)
        else:
            pred_x_start = model_prediction
...
def extract(a, t, x_shape):
    ...
    # Gather the coefficient values from `a` based on `t`
    out = a.gather(-1, t)
    out = out.reshape(
        b, *((1,) * (len(x_shape) - 1))
